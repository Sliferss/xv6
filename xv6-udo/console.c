...
#define vga3TotalAddress 2000
ushort vga3Snapshot[VGA_0x03_HEIGHT * VGA_0x03_WIDTH]; //text mode snapshot array
int cursorPos;                                         //cursor position snapshot

char Clearscreen_buffer[4][640*480/8];
...
static void cgaputc(int c) {
    ...
    if (currentvgamode != 0x03) //any textmode changes while in another mode will change the pointer to the array and cursor position to update the snapshot
    {
        crt = vga3Snapshot;
        pos = cursorPos;
    }
    ...
    {
        cursorPos = pos; //cursor position updated after changed so it loads new line correctly
    }
}
...
int consolevgamode(int vgamode) {
    ...
    {
        case 0x03: {
            conClearScreen(); //clear screen is before writevideo register so it can work as text mode has different memory adress and uses fonts instead of pixels
            writeVideoRegisters(registers_80x25_text);
            writeFont(font_8x16, 16);

            currentvgamode = 0x03;
            errorcode = 0;
            loadSnapshot(); //loads up snapshot 
        } break;

        case 0x12: {
            if (currentvgamode == 0x03) //check if in text mode to make snapshot otherwise does not change
            {
                saveSnapshot();
            }
            ...
            currentvgamode = 0x12;
            errorcode = 0;
            conClearScreen12(); //makes clean canvas when changing modes as new mode will be full of colours
        } break;
        ...
        case 0x13: {
            ...
            currentvgamode = 0x13;
            errorcode = 0;
            conClearScreen13(); //makes clean canvas when changing modes as new mode will be full of colours
        } break;
    }
...
int conDrawPixel(int x, int y, int colour)
{
    
    if (currentvgamode == 0x13) //checks vgamode for correct code
    {
        if(colour < 0 || colour > 15){
            return -1; //invalid colour check
        }

        else if (x < 0 || x > VGA_0x13_WIDTH || y < 0 || y > VGA_0x13_HEIGHT)
        {
            return -1; //check if pixel is within the screen space
        }
        else {
            uchar* vgaMem = VGA_0x13_MEMORY + VGA_0x13_WIDTH * y + x;
            *vgaMem = colour;
        }
        return 0;
    }
    else
    {
        ...
        else if (x < 0 || x > VGA_0x12_WIDTH || y < 0 || y > VGA_0x12_HEIGHT)
        {
            return -1; //check if pixel is within the screen space
        }

        int pixel_logical_offset = VGA_0x12_WIDTH * y + x;           //pixel position

        int pixel_bit_offset = 7 - pixel_logical_offset % 8;         //bit position

        int pixel_byte_offset = pixel_logical_offset /= 8;           //byte position

        int colour_position = 0x1;
        int mask = 0x0;

        for (int i = 0; i < 4; i++) {                                //loops through each plane as each needs to be changed
            consolevgaplane(i);                                      //sets the plane
            uchar* vgaMem = consolevgabuffer() + pixel_byte_offset;  //sets points to plane specific memory adress

            int bit_value = colour & colour_position;
            if (bit_value == 0) {
                mask = colour_position << pixel_bit_offset;
                *vgaMem = *vgaMem & ~(mask);
            } else {
                mask = bit_value << pixel_bit_offset;
                *vgaMem = *vgaMem | mask;
            }
            colour = colour >> 1;
        }
...
void conClearScreen()
{
    if(currentvgamode == 0x13)                                       //checks correct vga mode
    {
        conClearScreen13();
    }
    else
...
{
    uchar* vgaMem = VGA_0x13_MEMORY;
    memset(vgaMem, 0, VGA_0x13_HEIGHT * VGA_0x13_WIDTH);             //most efficient way to clear screen
}
...
void conClearScreen12()
{
    ...
    {
        consolevgaplane(i);
        uchar* vgaMem = consolevgabuffer();
        memset(vgaMem, 0, (VGA_0x12_WIDTH * VGA_0x12_HEIGHT) / 8);  //most efficient way to clear screen, must go through each plane
    }
}

void saveSnapshot()
{
    ushort* vga3Mem = (ushort*)VGA_0x03_MEMORY;

    for (int row = 0; row < VGA_0x03_HEIGHT; row++)                 //loops through each points in mode 3h
    {
        for (int col = 0; col < VGA_0x03_WIDTH; col++)
        {
            vga3Snapshot[VGA_0x03_WIDTH * row + col] = *vga3Mem;    //saves to global array for snapshot and increments position
            vga3Mem++;
        }
    }
    ...
    //code from cgaputc locate cursor position and save.
}
...
void loadSnapshot()
{
    ...
    for (int row = 0; row < VGA_0x03_HEIGHT; row++)
    {
        for (int col = 0; col < VGA_0x03_WIDTH; col++)
        {
            *vga3Mem = vga3Snapshot[VGA_0x03_WIDTH * row + col];    // inverse of savesnapshot
            vga3Mem++;
        }
    }
    ...
    //code from cgaputc place cursor at cursorPos.
}